...
my %CONTAINERS = (
    ...
    recycling_240 => 15,
    recycling_360 => 16,
    recycling_bag => 22, # blue stripe bag?
    paper_180 => 26,
    paper_240 => 27,
    paper_360 => 28,
    paper_bag => 34,
...
sub waste_containers {
    return {
        ...
        $CONTAINERS{recycling_bag} => 'Recycling Purple Bag', # Blue stripe bag?
...
sub image_for_unit {
    ...
    return svg_container_sack('normal', '#3B3B3A') if $service_id eq $SERVICE_IDS{fas_refuse} && $time_banded;
    if (my $container = $unit->{request_containers}[0]) {
        return svg_container_sack('normal', '#BD63D1') if $container == $CONTAINERS{recycling_bag};
    }
...
sub munge_bin_services_for_address {
    ...
    foreach (@$rows) {
        if ($_->{service_id} eq $SERVICE_IDS{domestic_food}) {
